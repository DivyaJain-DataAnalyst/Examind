              ...
              {tests.filter(isAvailable).length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {tests.filter(isAvailable).map((test) => (
                    <div key={test._id} className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md">
                      <div className="p-5">
                        ...
                        <div className="pt-2 border-t border-gray-100">
                          {hasAttempted(test._id) ? (
                            <Link to={`/student/results/${test._id}`} className="w-full inline-flex justify-center items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-md">
                              View Results
                            </Link>
                          ) : (
                            <Link to={`/student/take-test/${test._id}`} className="w-full inline-flex justify-center items-center px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-md">
                              <BookOpen size={16} className="mr-2" /> Start Test
                            </Link>
                          )}
                        </div>
                      </div>
