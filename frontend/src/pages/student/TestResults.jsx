...
import { mockTests, getQuestionsForTest, mockTestAttempts } from '../../data/mockData';
...
const TestResults = () => {
    ...
    const fetchResultsData = async () => {
      try {
        ...
        if (location.state?.answers) {
          ...
          const attempt = mockTestAttempts.find(a => a.testId === testId);
  ...
  const getAnswerLabel = (question) => {
    ...
    if (question.type === QUESTION_TYPES.MULTIPLE_CHOICE) {
      const option = question.options.find(opt => opt.id === answers[question.id]);
      return option ? `${answers[question.id].toUpperCase()}. ${option.text}` : answers[question.id];
    } else {
  ...
  const getCorrectAnswerLabel = (question) => {
    if (question.type === QUESTION_TYPES.MULTIPLE_CHOICE) {
      ...
      return option ? `${question.correctAnswer.toUpperCase()}. ${option.text}` : question.correctAnswer;
    } else {
      return `${question.correctAnswer} ${question.tolerance ? `(±${question.tolerance})` : ''}`;
    }
  };
    ...
    <div className="w-screen h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mt-2">Results: {test?.title}</h1>
        </div>
        
        <div className="bg-white shadow rounded-lg p-6 mb-8 fade-in">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-blue-50 p-6 rounded-lg text-center">
              <h2 className="text-lg font-medium text-gray-900 mb-2">Score</h2>
              <div className="text-3xl font-bold text-blue-600">{score} / {totalPoints}</div>
              <div className="text-sm text-gray-600 mt-1">{getScorePercentage()}%</div>
            </div>
            
            <div className="bg-gray-50 p-6 rounded-lg text-center">
              <h2 className="text-lg font-medium text-gray-900 mb-2">Grade</h2>
              <div className={`text-3xl font-bold ${getGradeColor()}`}>{getGrade()}</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className={`p-3 rounded-md ${
                      ...
                      {answers[question.id] ? getAnswerLabel(question) : 'Not answered'}
                    </div>
                  </div>
